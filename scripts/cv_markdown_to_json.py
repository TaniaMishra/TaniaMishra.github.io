...
def extract_author_info(config):
    ...
    if 'author' in config:
        ...
        # Add research areas to summary if available
        if author.get('research_areas'):
            if author_info['summary']:
                author_info['summary'] += f". {author.get('research_areas')}"
            else:
                author_info['summary'] = author.get('research_areas')
